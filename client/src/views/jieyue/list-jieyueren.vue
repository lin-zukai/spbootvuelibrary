    ...
    <div class="v-list" v-loading="loading" element-loading-text="加载中">
        <el-card class="box-card">
            ...
            <div class="form-search">
                <el-form @submit.prevent.stop :inline="true" size="mini">
                    ...
                    <el-form-item label="借阅人">
                        <el-input v-model="search.jieyueren"></el-input>
                    </el-form-item>
                    <el-form-item>
            ...
            <el-table border :data="list" style="width: 100%" highlight-current-row>
                ...
                <el-table-column label="借阅时间" width="120">
                    <template slot-scope="{row}"> {{ row.jieyueshijian }} </template>
                </el-table-column>
                <el-table-column label="归还时间" width="120">
                    <template slot-scope="{row}"> {{ row.guihuanshijian }} </template>
                </el-table-column>
                <el-table-column label="借阅天数">
                    <template slot-scope="{row}"> {{ row.jieyuetianshu }} </template>
                </el-table-column>
                <el-table-column label="借阅金额" width="80">
                    <template slot-scope="{row}"> {{ row.jieyuejine }} </template>
                </el-table-column>
                <el-table-column label="借阅状态" width="80">
                    <template slot-scope="{row}"> {{ row.jieyuezhuangtai }} </template>
                </el-table-column>
                <el-table-column label="借阅人" width="80">
                    <template slot-scope="{row}"> {{ row.jieyueren }} </template>
                </el-table-column>

                <el-table-column label="是否支付" width="120">
                    <template slot-scope="{row}">{{ row.iszf }}
                        <el-button
                            size="mini"
                          type="primary"
                            @click="$goto({path:'/admin/pay' , query:{biao:'jieyue' , id:row.id,ordersn:row.jieyuebianhao,zongji:row.jieyuejine } })"
                            v-if="row.iszf=='否' && row.meitianjiage >0">
                            去支付
                        </el-button>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作">
                    <template slot-scope="{row}">
                        <el-button-group>
                            ...
                            <el-button @click="$goto('/admin/guihuanadd?id='+row.id)" v-if="row.jieyuezhuangtai == '借阅中'"  type="success" icon="el-icon-plus" size="mini"> 图书归还 </el-button>

                            <el-tooltip content="详情" placement="top">
                                <el-button @click="$goto({path:'/admin/jieyuedetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                            </el-tooltip>
            ...
            <div>支出金额总和: {{ total.sum_jieyuejine }}</div>
        </el-card>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                this.$post(api.jieyue.listjieyueren, filter)
                    .then((res) => {
                        this.loading = false;
