        ...
        <el-card class="box-card">
            ...
            <el-table border :data="list" style="width: 100%" highlight-current-row>
                ...
                <el-table-column label="举报内容">
                    <template slot-scope="{row}"> {{ row.jubaoneirong }} </template>
                </el-table-column>
                <el-table-column label="举报人" width="80">
                    <template slot-scope="{row}"> {{ row.jubaoren }} </template>
                </el-table-column>
                <el-table-column label="举报状态" width="130">
                    <template slot-scope="{row}"> {{ row.jubaozhuangtai }} </template>
                </el-table-column>
                ...
                <el-table-column label="操作">
                    <template slot-scope="{row}">
                        <el-button-group>
                            ...
                            <el-button @click="$goto('/admin/jubaochuliadd?id='+row.id)" v-if="row.jubaozhuangtai == '待处理'" type="success" icon="el-icon-plus" size="mini"> 举报处理 </el-button>

                            <el-tooltip content="详情" placement="top">
                                <el-button @click="$goto({path:'/admin/jubaodetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                            </el-tooltip>
                            <el-tooltip content="编辑" placement="top">
                                <el-button icon="el-icon-edit" @click="$goto({path:'/admin/jubaoupdt',query:{id:row.id } })" type="warning" size="mini"></el-button>
                            </el-tooltip>
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                this.$post(api.jubao.list, filter)
                    .then((res) => {
                        this.loading = false;
