            ...
            <el-table border :data="list" style="width: 100%" highlight-current-row>
                ...
                <el-table-column label="图书分类" width="80">
                    <template slot-scope="{row}">
                        <e-select-view module="tushufenlei" :value="row.tushufenlei" select="id"
                ...
                <el-table-column label="视频">
                    <template slot-scope="{row}">
                        <e-video-list :src="row.shipin"></e-video-list>
                    </template>
                </el-table-column>
                ...
                <el-table-column width="120" label="审核">
                    <template slot-scope="{row}">
                        ...
                        <el-button size="mini" :type="row.issh == '是' ? 'success' : 'danger'"
                                   @click="checkIssh(row,'tushuxinxi')">
                            {{ row.issh == '是' ? '取消审核' : '审核' }}
                        </el-button>
                    </template>
                </el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                this.$post(api.tushuxinxi.list, filter)
                    .then((res) => {
                        this.loading = false;
