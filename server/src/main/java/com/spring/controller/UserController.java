...
public class UserController extends BaseController {
    ...
    protected String authLoginUser(boolean isAdmin, String username, String pwd, String cx) {
        ...
        if (cx.equals("图书管理员")) {
            table = "tushuguanliyuan";
            Tushuguanliyuan user = tushuguanliyuanService.login(username, pwd);
            if (user == null) {
                return showError("用户名或密码错误");
            }
            session.setAttribute("id", user.getId());
    ...
    public String editPassword() {
        ...
        if (cx.equals("图书管理员")) {
            Tushuguanliyuan user = tushuguanliyuanService.login(username, oldPassword);
            if (user == null) {
                return showError("原密码不正确");
            }
            tushuguanliyuanService.updatePassword(user.getId(), newPwd);
        }
