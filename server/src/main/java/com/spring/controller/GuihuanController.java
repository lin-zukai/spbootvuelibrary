 ...
 * 归还 */
@Controller
public class GuihuanController extends BaseController {

    @Autowired
    private GuihuanMapper dao;

    @Autowired
    private GuihuanService service;

    @Autowired
    private JieyueService serviceRead;

    /**
     *  后台列表页
     *
     */
    @RequestMapping("/guihuan_list")
    public String list() {
        // 检测是否有登录，没登录则跳转到登录页面
        if (!checkLogin()) {
        ...
        Example example = new Example(Guihuan.class); //  创建一个扩展搜索类
        ...
        List<Guihuan> list = service.selectPageExample(example, page, pagesize); // 获取当前页的行数
    ...
    public String getWhere() {
        ...
        if (!Request.get("huanshuren").equals("")) {
            where += " AND huanshuren LIKE '%" + Request.get("huanshuren") + "%' ";
        }
        return where;
    }
     ...
     * 发布人列表
     */
    @RequestMapping("/guihuan_list_faburen")
    public String listfaburen() {
        ...
        List<Guihuan> list = service.selectPageExample(example, page, pagesize);
        assign("totalCount", request.getAttribute("totalCount"));
        assign("list", list);
     ...
     * 借阅人列表
     */
    @RequestMapping("/guihuan_list_jieyueren")
    ...
    @RequestMapping("/guihuan_add")
    public String add() {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id"); // 根据id 获取 借阅模块中的数据
        Jieyue readMap = serviceRead.find(id);
        // 将数据行写入给前台jsp页面
    ...
    @RequestMapping("/guihuan_updt")
    public String updt() {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Guihuan mmm = service.find(id);
        assign("mmm", mmm);
        assign("updtself", 0);
    ...
    public String insert() {
        ...
        post.setShijiguihuanshijian(Request.get("shijiguihuanshijian"));

        post.setBeizhu(Request.get("beizhu"));

        post.setHuanshuren(Request.get("huanshuren"));

        post.setJieyueid(Request.getInt("jieyueid"));

        service.insert(post); // 插入数据
        int charuid = post.getId().intValue();
        Query.execute("update tushuxinxi set tushuzhuangtai='可借阅'  where tushubianhao='" + request.getParameter("tushubianhao") + "'");

        Query.execute("update jieyue set jieyuezhuangtai='已归还' where id='" + request.getParameter("jieyueid") + "'");
     ...
     * @return
     */
    @RequestMapping("/guihuanupdate")
    public String update() {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Guihuan post = new Guihuan();
        // 将前台表单数据填充到实体类
        if (!Request.get("jieyueid").equals("")) post.setJieyueid(Request.getInt("jieyueid"));
        ...
        if (!Request.get("shijiguihuanshijian").equals("")) post.setShijiguihuanshijian(Request.get("shijiguihuanshijian"));
        if (!Request.get("beizhu").equals("")) post.setBeizhu(Request.get("beizhu"));
        if (!Request.get("huanshuren").equals("")) post.setHuanshuren(Request.get("huanshuren"));
     ...
     *  后台详情
     */
    @RequestMapping("/guihuan_detail")
    public String detail() {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Guihuan map = service.find(id); // 根据前台url 参数中的id获取行数据
    ...
    public String delete() {
        ...
        Map map = Query.make("guihuan").find(id);
