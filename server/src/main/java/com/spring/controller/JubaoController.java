...
public class JubaoController extends BaseController {
    ...
    @Autowired
    private JubaoMapper dao;

    @Autowired
    private JubaoService service;
    ...
    public String list() {
        ...
        Example example = new Example(Jubao.class); //  创建一个扩展搜索类
        ...
        List<Jubao> list = service.selectPageExample(example, page, pagesize); // 获取当前页的行数
     ...
     * 发布人列表
     */
    @RequestMapping("/jubao_list_faburen")
    public String listfaburen() {
        ...
        List<Jubao> list = service.selectPageExample(example, page, pagesize);
        assign("totalCount", request.getAttribute("totalCount"));
        assign("list", list);
     ...
     * 举报人列表
     */
    @RequestMapping("/jubao_list_jubaoren")
    ...
    @RequestMapping("/jubao_updt")
    public String updt() {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Jubao mmm = service.find(id);
        assign("mmm", mmm);
        assign("updtself", 0);
     ...
     * @return
     */
    @RequestMapping("/jubaoinsert")
    public String insert() {
        _var = new LinkedHashMap(); // 重置数据
        String tmp = "";
        Jubao post = new Jubao(); // 创建实体类
        // 设置前台提交上来的数据到实体类中
        ...
        post.setJubaoneirong(Request.get("jubaoneirong"));

        post.setJubaoren(Request.get("jubaoren"));

        post.setJubaozhuangtai(Request.get("jubaozhuangtai"));
     ...
     * @return
     */
    @RequestMapping("/jubaoupdate")
    public String update() {
        _var = new LinkedHashMap(); // 重置数据
        // 创建实体类
        Jubao post = new Jubao();
        // 将前台表单数据填充到实体类
        ...
        if (!Request.get("jubaoneirong").equals("")) post.setJubaoneirong(Request.get("jubaoneirong"));
        if (!Request.get("jubaoren").equals("")) post.setJubaoren(Request.get("jubaoren"));
        if (!Request.get("jubaozhuangtai").equals("")) post.setJubaozhuangtai(Request.get("jubaozhuangtai"));
     ...
     *  后台详情
     */
    @RequestMapping("/jubao_detail")
    public String detail() {
        _var = new LinkedHashMap(); // 重置数据
        int id = Request.getInt("id");
        Jubao map = service.find(id); // 根据前台url 参数中的id获取行数据
    ...
    public String delete() {
        ...
        Map map = Query.make("jubao").find(id);
